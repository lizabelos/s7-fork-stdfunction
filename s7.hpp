...
typedef struct s7_pointer {
    ...
    template <typename T> inline s7_pointer(T ptr) : mPtr((s7_cell*)ptr) {}

    template <typename T> inline bool operator==(T *ptr) const { return mPtr == (void*)ptr; }
    template <typename T> inline bool operator!=(T *ptr) const { return mPtr != (void*)ptr; }
    template <typename T> inline bool operator<(T *ptr) const { return mPtr < (void*)ptr; }
    template <typename T> inline bool operator>(T *ptr) const { return mPtr > (void*)ptr; }
    template <typename T> inline bool operator<=(T *ptr) const { return mPtr <= (void*)ptr; }
    ...
    inline bool operator==(const s7_pointer &ptr) const { return mPtr == ptr.mPtr; }
    inline bool operator!=(const s7_pointer &ptr) const { return mPtr != ptr.mPtr; }
    inline bool operator<(const s7_pointer &ptr) const { return mPtr < ptr.mPtr; }
    inline bool operator>(const s7_pointer &ptr) const { return mPtr > ptr.mPtr; }
    inline bool operator<=(const s7_pointer &ptr) const { return mPtr <= ptr.mPtr; }
    ...
    inline bool operator==(std::nullptr_t) const { return mPtr == nullptr; }
    inline bool operator!=(std::nullptr_t) const { return mPtr != nullptr; }
    explicit inline operator bool() const { return mPtr; }

    inline s7_cell *operator->() { return mPtr; }
    inline const s7_cell *operator->() const { return mPtr; }
    inline s7_cell *get() { return mPtr; }
    inline const s7_cell *get() const { return mPtr; }

    s7_pointer operator+(int64_t i) const;
    inline s7_pointer operator++(int i) { assert(i == 0); s7_pointer copy = *this; *this = *this + 1; return copy; }
    inline s7_pointer operator++() { *this = *this + 1; return *this; }
...
typedef s7_pointer (*s7_function_c)(s7_scheme *sc, s7_pointer args);   /* that is, obj = func(s7, args) -- args is a list of arguments */
typedef s7_pointer (*s7_pfunc_c)(s7_scheme *sc);

#include <functional>
#include <memory>
#include <unordered_set>

extern std::unordered_set<std::function<s7_pointer(s7_scheme*, s7_pointer)>*> *s7_function_shared_cpp;

class s7_function {

public:
    inline s7_function() : mFunctionCpp(nullptr), mFunctionC(nullptr), mPFunctionC(nullptr) {};

    inline s7_function(s7_function_c f) : mFunctionCpp(), mFunctionC(f), mPFunctionC(nullptr) {};

    inline s7_function(s7_pfunc_c f) : mFunctionCpp(), mFunctionC(nullptr), mPFunctionC(f) {};

    inline s7_function(std::function<s7_pointer(s7_scheme*, s7_pointer)> f) : mFunctionCpp(new std::function<s7_pointer(s7_scheme*, s7_pointer)>(f)), mFunctionC(nullptr) {
        if (s7_function_shared_cpp == nullptr) {
            s7_function_shared_cpp = new std::unordered_set<std::function<s7_pointer(s7_scheme*, s7_pointer)>*>();
        }
        s7_function_shared_cpp->insert(mFunctionCpp);
    };

    inline s7_function(s7_pointer f) : mFunctionCpp(), mFunctionC((s7_function_c)f.get()), mPFunctionC((s7_pfunc_c)f.get()) {
        if (s7_function_shared_cpp != nullptr) {
            auto cppf = (std::function<s7_pointer(s7_scheme*, s7_pointer)>*)f.get();
            if (s7_function_shared_cpp->find(cppf) != s7_function_shared_cpp->end()) {
                mFunctionCpp = cppf;
                mFunctionC = nullptr;
                mPFunctionC = nullptr;
            }
    ...
    inline s7_pointer operator()(s7_scheme *sc) {
        if (mPFunctionC != nullptr) {
            return mPFunctionC(sc);
        } else {
            throw std::runtime_error("s7_function: no pfunction set");
        }
    }

    inline s7_pointer operator()(s7_scheme *sc, s7_pointer args) {
        if (mFunctionCpp) {
            return (*mFunctionCpp)(sc, args);
        } else if (mFunctionC != nullptr) {
            return mFunctionC(sc, args);
        } else {
            throw std::runtime_error("s7_function: no function set");
        }
    }

    inline s7_pointer ptr() const {
        if (mFunctionCpp) {
            return (s7_pointer) mFunctionCpp;
        } else if (mFunctionC != nullptr) {
            return (s7_pointer) mFunctionC;
        } else {
            return mPFunctionC;
        }
    }

    inline bool operator==(const s7_pointer &other) const { return ptr() == other.get(); }
    inline bool operator!=(const s7_pointer &other) const { return ptr() != other.get(); }
    explicit inline operator bool() const { return ptr().get(); }

private:
    std::function<s7_pointer(s7_scheme*, s7_pointer)>* mFunctionCpp;
    s7_function_c mFunctionC;
    s7_pfunc_c mPFunctionC;
};

template <> inline s7_pointer::s7_pointer(s7_function ptr) : mPtr((s7_cell*)ptr.ptr().get()) {};

using s7_pfunc = s7_function;
...
template <typename ...Args> s7_pointer s7_make_signature(s7_scheme *sc, s7_int len, Args ...args);               /* procedure-signature data */
template <typename ...Args> s7_pointer s7_make_circular_signature(s7_scheme *sc, s7_int cycle_point, s7_int len, Args ...args);
s7_pointer s7_make_signature2(s7_scheme *sc, s7_int len, s7_pointer arg1, s7_pointer arg2);
